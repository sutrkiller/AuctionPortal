...
namespace BL.Queries
{
    public class AuctionListQuery : AppQuery<AuctionDTO>
    {
        ...
        public IQueryable<AuctionDTO> GetFiltered()
        {
            ...
            if (Filter.HighestBidderId > 0)
            {
                query =
                    query.Where(
                        x =>
                            x.Bids.Count > 0 &&
                            x.Bids.OrderByDescending(b => b.BidTime).FirstOrDefault().User.ID == Filter.HighestBidderId);
            }
            if (Filter.BidderId > 0)
            {
                query = query.Where(x => x.Bids.Any(b => b.User.ID == Filter.BidderId));
            }

           //query= query.OrderBy(a => DateTime.Now > a.AuctionEnd || (a.Bids.Any() &&  a.Bids.OrderByDescending(x=>x.Value).FirstOrDefault().Value >= a.BasePrice * 100m ) ? 1 : 0);
